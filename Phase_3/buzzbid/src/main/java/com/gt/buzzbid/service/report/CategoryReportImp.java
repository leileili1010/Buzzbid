...
public class CategoryReportImp implements CategoryReportService{
    ...
    public List<CategoryReportModel> getCategoryReport() {
        ...
        String query="SELECT c.category as category,COUNT(i.*) as total_items,MIN(a.get_it_now_price) AS min_price,MAX(a.get_it_now_price) AS max_price,ROUND(avg(a.get_it_now_price), 2) as avg_price " +
                "FROM Item i JOIN Category c ON i.category_id = c.category_id JOIN Auction a ON i.item_id = a.item_id " +
        ...
        try {
            ...
            while (resultSet.next()) {
                categoryReport.add(new CategoryReportModel(resultSet.getString("category"),
                        ...
                        "$"+resultSet.getString("min_price"),
                        "$"+resultSet.getString("max_price"),
                        "$"+resultSet.getString("avg_price")));
            }
        } catch (Exception e) {
